...
| pltr-latest-beneficial-ownership-snapshot | simple | summarize_beneficial_ownership | What does Palantir's latest beneficial ownership filing say about the filer, the reported ownership percentage, and whether the position is passive or activist? |
| para-recent-beneficial-ownership-filings | medium | get_beneficial_ownership | Since 2025-01-01, what recent beneficial ownership filings has Warner Bros. Discovery had? List the form type, filing date, and reported ownership percentage for the recent filings. |
| wmt-latest-8k-section-list | medium | list_filings, list_filing_sections | What item sections are included in Walmart's latest 8-K? Give the accession number and list the item codes with their headings. |
| unh-latest-10k-risk-section | medium | list_filings, list_filing_sections, read_filing_section | In UnitedHealth's latest 10-K Item 1A, what are two risk themes management highlights? Cite the accession number. |
| aapl-latest-10k-mdna-highlights | medium | list_filings, list_filing_sections, read_filing_section | In Apple's latest 10-K MD&A, what are two management highlights? Cite the accession number. |
| nke-latest-10k-china-search | medium | list_filings, search_filing_text | In Nike's latest 10-K, what does management mention about China? Cite the accession number and the section where the match appears. |
...
| ups-risk-factors-change | complex | list_filings, compare_filing_sections | How did UPS's latest annual risk factor section change versus the prior annual filing? Identify one new emphasis and cite both accession numbers. |
...
- The expanded corpus now exercises the newer SEC tools for beneficial ownership, section discovery, targeted section reads including MD&A, filing search, and filing-to-filing section comparison.
- Filing-heavy questions such as nvda-latest-10k-risk-themes, brk-top-13f-holdings, the new section/search/comparison questions, and the proxy-heavy cases are not in the smoke subset because they depend more heavily on local EDGAR cache coverage.
...
- The live harness defaults to 300-second budgets for evidence collection, chat generation, and evaluator grading per question.