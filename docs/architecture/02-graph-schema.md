...
> **Original Source Storage**
>
> Every ingested document is saved locally before processing. SEC filings, news articles, analyst reports, press releases — all raw source files are written to disk under `data/raw/` and the local path is stored on the corresponding graph node (`file_path`). This enables:
> - **Re-processing**: re-run LLM extraction with improved prompts without re-fetching
> - **Auditability**: verify exactly what source text produced a given extracted relationship
...
(:Company {
  ...
  legal_name: STRING,      -- "Apple Inc." (from SEC filings — used for entity resolution)
  status: STRING,          -- "active" | "acquired" | "delisted" | "bankrupt"

  -- Minimal snapshot for in-graph filtering
  market_cap: FLOAT,       -- Current market cap in USD (refreshed from price feed; enables
                           --   size-based Cypher filters during graph traversal)

  -- LLM-generated investment intelligence (generated from 10-K text; refreshed on each new filing)
  summary: STRING,         -- What the company does, its business model, and competitive position
  risk_factors: STRING,    -- Key investment risks distilled from the 10-K Risk Factors section
  opportunities: STRING,   -- Growth catalysts, strategic initiatives, and tailwinds
...
> **`market_cap` snapshot**: The only financial value stored directly on the Company node. Its purpose is narrow: enabling size-based filters (`WHERE c.market_cap > 1e9`) *inside* Cypher graph traversals without a round-trip to DuckDB. All other financial metrics (revenue, EPS, P/E, margins, growth rates) live exclusively in DuckDB and are accessed via the `get_financial_history` agent tool. Updated from the price feed on each ingestion cycle.

> **LLM-generated intelligence fields**: `summary`, `risk_factors`, and `opportunities` are generated by LLM from 10-K filing text at ingestion time. They are **re-generated every time a significant new event is processed** for the company — currently triggered by a new filing (10-K or 10-Q), and in future phases by material news, analyst reports, or macro events. The generation prompt receives the new event's content plus the existing summary as context, so each update is incremental rather than a from-scratch rewrite. These fields are the primary source of qualitative investment context for the agent system.
...
(:Person {
  ...
  bio: STRING,             -- LLM-generated summary of background and investment relevance,
                           --   extracted from DEF 14A, press releases, and public filings
  last_updated: STRING
})
```
...
(:Filing {
  ...
  key_topics: [STRING],      -- Extracted key topics (e.g. ["supply chain", "tariffs", "AI spend"])
  sentiment: FLOAT,          -- -1.0 to 1.0 (tone of the filing overall)
  summary_embedding: VECTOR, -- For semantic search across filings
  last_updated: STRING
})
```

> **Pipeline state** (`processed`, `fetch_status`, etc.) is tracked in SQLite `filing_state` — not on the Filing node. The graph only stores investment-relevant data.

### Industry
Industry classifications (GICS-aligned). Reference taxonomy node — agents use it for peer grouping and sector filtering.

```
(:Industry {
  name: STRING,            -- "Consumer Electronics"
  gics_code: STRING,       -- GICS industry code
  description: STRING      -- Brief description of what companies in this industry do
})
```
...
- **NewsArticle** — news from Reuters, Bloomberg, FMP news feed; key fields: `article_id`, `title`, `source`, `url`, `file_path` (locally cached copy at `data/raw/news/{source}/{article_id}.html`), `published_date`, `summary` (LLM-generated), `sentiment`, `impact_score`, `embedding`; linked to companies via `MENTIONED_IN`
...
- **ResearchReport** — analyst reports and agent-generated investment theses; key fields: `report_id`, `title`, `source` ("Goldman Sachs", "agent"), `file_path` (locally cached at `data/raw/reports/{source}/{report_id}.pdf`), `published_date`, `summary` (LLM-generated), `rating`, `price_target`, `embedding`; linked to companies, filings, news
...
> **Graph traversal + size filtering** uses `market_cap` directly in Cypher:
> ```cypher
> -- Find mid-cap companies in Apple's supply chain
> MATCH (apple:Company {ticker: 'AAPL'})<-[:SUPPLIES_TO]-(supplier:Company)
> WHERE supplier.market_cap < 10e9
> RETURN supplier.ticker, supplier.name, supplier.market_cap
> ```
> **Financial screening queries** (P/E, revenue growth, margins) require a DuckDB tool call — they are not stored on the graph node. The agent combines a graph traversal result with a `get_financial_history` call to apply financial filters.
>
> **Trend/time series queries** hit DuckDB exclusively. See [Time Series Data Store](#time-series-data-store-duckdb).
...
CALL db.idx.fulltext.createNodeIndex('company_search', 'Company', 'name', 'summary')
...
```
   ...
   │ (4M+ rows) │   │ relations  │
   │            │   │            │
   │ Growth     │──►│ market_cap │  Recompute Company.market_cap from price feed
   │ calcs      │   │ on Company │  (all other metrics stay in DuckDB only)
   └────────────┘   └────────────┘
```

Agent tools query both stores:
- **Graph traversal + size filtering** → FalkorDB `market_cap` snapshot (fast Cypher, single financial filter)
- **Trend analysis / growth rates / financial screening** → DuckDB (`get_financial_history` tool)
